...
import kr.baka.groupriding.databinding.DialogGroupRidingFinishBinding
...
import kr.baka.groupriding.viewmodel.GroupRidingFinishViewModel
import java.util.*
import kotlin.collections.ArrayList


class GroupRidingFinishDialog(context: Context, private val latLngs: ArrayList<LatLng>) : Dialog(context) {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        requestWindowFeature(Window.FEATURE_NO_TITLE)   //타이틀바 제거
        setCancelable(true) //다이얼로그의 바깥 화면을 눌렀을 때 다이얼로그가 닫히지 않도록 함

        val binding: DialogGroupRidingFinishBinding = DataBindingUtil.inflate(
            LayoutInflater.from(context),
            R.layout.dialog_group_riding_finish,
            null,
            false
        )
        setContentView(binding.root)
        val viewModel = GroupRidingFinishViewModel()
